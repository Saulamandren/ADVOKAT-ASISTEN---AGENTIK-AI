# agents/perdata_agent.py

from langchain_ollama.llms import OllamaLLM
from retriever.retriever_tool import perdata_retriever
...
prompt_template = """
Anda adalah seorang Advokat AI, ahli Hukum Perdata di Indonesia.
Tugas Anda adalah menganalisis kasus hukum berdasarkan pasal-pasal KUHPerdata yang relevan.
...
# 3. Buat Rantai (Chain) Analisis Perdata
#    Chain ini siap untuk diimpor dan digunakan oleh agen router utama.
perdata_chain = (
    {"context": RunnableLambda(lambda x: perdata_retriever(x['question'])), "question": RunnablePassthrough()}
    | prompt
    | llm
)
